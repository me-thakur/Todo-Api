...
app.delete('/todos/:id', (req, res) => {
  ...
  Todo.findByIdAndDelete(id).then((todo) => {
    if (!todo) {
      return res.status(404).send(id2);
    }
    res.send({todo});
  }).catch((e) => {
    res.status(400).send();
...
app.patch('/todos/:id', (req, res) => {
  var id = req.params.id;
  var body = _.pick(req.body, ['text', 'completed']); // only things we want to update by user
  ...
  if (_.isBoolean(body.completed) && body.completed) {
    body.completedAt = new Date().getTime(); //january 1st midnight is called as Unix epoc
  } else {
